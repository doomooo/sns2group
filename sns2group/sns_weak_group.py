...
#群组热度
group_relation=[get_relation(group_date[x]) for x in range(len(group_date))]


#群组对于亲密度的增量
def get_relation_from_group(p,other_p):
    relation=0;
    for i in range(len(weak_group)):
        relation+=group_relation[i]/len(weak_group[i])*(reduce(lambda x,y:x+(0,1)[y in weak_group[i]],other_p,0) ,0)[p in weak_group[i]]
    return relation

#获取两人之间的亲密度
p2p_relation=[[(get_relation(p2p_date[x][y])+get_relation_from_group(x,[y]),0)[x==y]  for y in range(len(p2p_date))] for x in range(len(p2p_date))]
            ...
            for group_person_i in list(set(all_person)-group_set[i]):    #扩充人选不在圈子内，且在150人之中
                for group_i in range(i):
                    ...
                    if group_set[i]<=group_set[group_i]:                #判断组合成的新圈子是否在之前的圈子中，是的话，则换下一个
                        ...
                        group_set[i].remove(group_person_i)
                all_relation=reduce(lambda x,y:x+p2p_relation[group_person_i][y],list(group_set[i]),0)/len(group_set[i])   #计算扩从人选的总亲密度
                all_relation+=get_relation_from_group(group_person_i,list(group_set[i]))
